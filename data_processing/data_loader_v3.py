...
class DataLoader:
    ...
    def __init__(self, 
        ...
        if task_rep_path is not None:
            with open(task_rep_path, 'rb') as handle:
                self.task_rep_data = pickle.load(handle)
    ...
    def load_task_rep_batch(self):
        """Yields a batch of Grasp training data -- (img_rgbd, colour_r, 
            colour_g, colour_b, colour_a, location_x, location_y)."""
        ...
        for i, (img, colour_r, colour_g, colour_b, colour_a, location_x, location_y)\
            ...
            colour_r_batch.append(colour_r)
            colour_g_batch.append(colour_g)
            colour_b_batch.append(colour_b)
            ...
            if (i + 1) % self.batch_size == 0:
                yield (torch.stack(img_batch),
                    ...
                    torch.tensor(colour_g_batch, dtype=torch.float32),
    ...
    def load_task_rep(self, include_depth=True):
        """Yields a single instance of Task Rep training data -- (img, grasp-map)."""
        for i, img_id_with_var in enumerate(self.img_id_list):
            img_angle = int(img_id_with_var.split('_')[-1])
            img_id = img_id_with_var.split('_')[-2]
            img_var = img_id_with_var.split('_')[0]
            img_name = img_var + '_' + img_id
            ...
            colour_r = self.task_rep_data['colour_r'][i]
            colour_g = self.task_rep_data['colour_g'][i]
            colour_b = self.task_rep_data['colour_b'][i]
