...
def fix_labels(mylabels, tooclose=0.1, sepfactor=2):
    """
    Adjust percentage labels on pie charts to hug the cirlce better.
    """
...
def plot_transition_chi(save_path='chi_per_transition_significance.png',
    ...
    threshes = {'top_k':[20,30], 'effect_thresh': [0.15,0.5], 'ratio':[0.3, 0.3]}

    plt.figure(figsize=(7,4))
    plt.axhline(y=0, color='gray', linestyle='dashed')
    for thresh_i, thresholding in enumerate(['strict', 'liberal']):
        all_results = run_chi_square_specialization(
        ...
        counts_on='base',
        weight=weight,
        top_k = threshes['top_k'][thresh_i],
        effect_thresh=threshes['effect_thresh'][thresh_i],
        ratio_high=threshes['ratio'][thresh_i],
        ratio_low=threshes['ratio'][thresh_i]
        )
        ...
        for res in all_results:
            ...
            plt.plot(x, chi_vals, marker='o', linewidth=2, markersize=6, label=res['method'], color=color,linestyle=f'{'dashed' if not thresh_i else 'solid'}')
    ...
    plt.title(title)
    handles = [
        Line2D([0], [0], color='#B5651D', linestyle='dashed',
               linewidth=2, markersize=10, label='Top K (k=20)'),
        Line2D([0], [0], color='#B5651D', linestyle='solid',
               linewidth=2, markersize=6, label='Top K (k=30)'),
        Line2D([0], [0], color='#006400', linestyle='dashed',
               linewidth=2, markersize=6, label='Effect (d=0.15)'),
        Line2D([0], [0], color='#006400', linestyle='solid',
               linewidth=2, markersize=6, label='Effect (d=0.5)'),
    ]
    ax = plt.gca()
    ax.legend(handles=handles, frameon=False, loc='upper right', handlelength=2.5)
...
def normalize_shapley_sum_to_100(shap_values, layer_sizes, target_sum=100.0, force_positive=False, eps=1e-12):
    """
    Normalize per-layer, per-task Shapley values so sums are controlled.

    - force_positive=False: scale by a positive factor so sum has magnitude target_sum
      and preserves signs (negative sums -> -target_sum). Preserves ordering and signs.
    - force_positive=True: add a constant offset so the sum is +target_sum without flipping
      signs. Preserves ordering but may shift some values across zero.
    """
    out = shap_values.copy()
    L = len(layer_sizes)
    T = out.shape[2]
    for l in range(L):
        n = int(layer_sizes[l])
        for t in range(T):
            ...
            if force_positive:
                # Shift by a constant so sum is exactly +target_sum, preserve order and signs
                offset = (target_sum - s) / n
                out[l, :n, t] = v + offset
            else:
                # Scale by a positive factor; preserve signs and ranking
                scale = abs(target_sum) / (abs(s) + eps)
                out[l, :n, t] = v * scale
    return out
def compare_specialized_edge_to_shapley():
    ...
    conn_shap_values = normalize_shapley_sum_to_100(conn_shap_values, C_SIZES, target_sum=100.0, force_positive=True)
    ...
    for layer_i in range(4):
        ...
        axL.set_ylabel(f"Layer {layer_i+1} -> {layer_i+2}\nConnection Shapley Score")
    ...
    fig.suptitle("Average Shapley Score per Connection Type")
...
def run_chi_square_specialization(graph_path='graphs/just_weights.pickle',
                                  ...
                                  counts_on='base',
                                  weight=True,top_k=20,effect_thresh=0.15, ratio_high = 0.3, ratio_low=0.3):
    """
    ...
    methods_to_run = ['topk_each','effect']
    ...
    for m in methods_to_run:
        ...
        node_types = build_node_types_advanced(shap_values, method=m, top_k=top_k, \
            effect_thresh=effect_thresh, ratio_high=ratio_high, ratio_low=ratio_low)
...
if __name__ == "__main__":
    ...
    cross = True
    remove_cross = True
    plot_transition_chi(save_path=f'vis/chi2/remove_cross_{'weight' if weight else 'count'}_{'cross_' if cross else ''}chi_per_transition_significance.png', 
