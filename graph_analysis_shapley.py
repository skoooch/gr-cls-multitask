...
def build_connectivity_graph(graph, model):
    shap_scores = np.load("shap_values.npy")
    for layer_idx in range(len(layers) - 1):
        ...
        for i in range(layer_X.weight.data.shape[0]):
            for j in range(layer_Y.weight.data.shape[0]):
                ...
                tgt = f"{name_Y}_k{j}"
                if weight:
                    graph.add_edge(src, tgt, weight=shap_scores[layer_idx, i,0] * shap_scores[layer_idx + 1, j,0])
...
pickle.dump(graph, open('shap_graph_unnormalized.pickle', 'wb'))
