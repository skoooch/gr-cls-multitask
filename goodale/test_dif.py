...
for width in range(30, 120,5):
    for height in range(30, 120,10):
        ...
        data_points.append((width, height, outputs[0][0].detach().cpu()))
print(data_points)
x_values = np.array([float(x) for x, y, z in data_points])
#y_values = np.array([float(y.item()) for x, y, z in data_points])
z_values = np.array([float(z) for x, y, z in data_points])
fig = plt.figure()
ax = fig.add_subplot()
...
plt.ylabel('Predicted Width (pixels)')
plt.xlabel('Actual Width(pixels)')
plt.title(f'{model_type} model')
plt.legend()
plt.grid(True)
print("here")
plt.savefig(f"./goodale/loss_{model_type}_center.png")
plt.clf()
